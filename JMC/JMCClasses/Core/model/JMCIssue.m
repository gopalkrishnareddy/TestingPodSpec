...
- (id) initWithDictionary:(NSDictionary*)map
{
    ...
    if ((self = [super init])) {
        ...
        NSMutableString *s = [NSMutableString stringWithString:[lowerMap objectForKey:@"description"]];
        ...
        if (start.location != NSNotFound && end.location != NSNotFound) {
            ...
            self.description = s;
        }else if ([self.description rangeOfString:@"Incident Identifier:"].location != NSNotFound) {
            self.description = @"Submitted Crash Report 📄";
        }
        ...
        if (!self.key)
        {
            self.key = @"(Queued for Submission)";
        }
        if (!self.status)
        {
