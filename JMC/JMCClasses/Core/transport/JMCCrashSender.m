...
- (void)sendCrashReports {
    ...
    NSString *defaultHeader = [[JMC sharedInstance] customFieldsAsPayloadString];
    
    // queue all the reports
    for (NSString *report in reports) {
        
        NSString *msg = [NSString stringWithFormat:@"%@%@", (defaultHeader ? : @"\n"), report];
        
        NSUInteger trimLength = [JMC maxCrashDescriptionLength];
        
        BOOL gt500 = [msg length] > trimLength;
        
        NSUInteger toIndex = gt500 == true ? trimLength : [msg length];
        
        [_transport send:@"Crash report" description:[[msg substringToIndex:toIndex] stringByAppendingString:(gt500 ? @"...\n(truncated)" : @"\n(end)")] crashReport:report];
    }
