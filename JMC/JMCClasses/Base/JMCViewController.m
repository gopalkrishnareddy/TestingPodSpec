...
- (IBAction)sendFeedback {
    ...
    if (self.replyToIssue) {
        ...
        NSString *customHeader = [[JMC sharedInstance] customFieldsAsPayloadString];
        
        NSString *description = self.descriptionField.text;
        
        NSUInteger trimLength = [JMC maxIssueSubjectTransmissionLength];
        
        NSUInteger toIndex = ([description length] > trimLength ? trimLength : [description length]);
        
        NSString *truncationMarker = ([description length] > trimLength ? @"..." : @"");
        
        [self.issueTransport send:[[description substringToIndex:toIndex] stringByAppendingString:truncationMarker]
                      description:[NSString stringWithFormat:@"%@%@", customHeader, self.descriptionField.text]
                      attachments:allAttachments];
    }
