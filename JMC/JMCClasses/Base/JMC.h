...
@interface JMC (PGDCore)

/**
 *  JMCSetupBlock for handling Async JMC Setup
 *
 *  @author                Andy Newby
 *  @param jmc     Instance of JMC
 *  @param success Successful Setup (YES/NO), Errors will be printed to logs.
 */
typedef void (^JMCSetupBlock)(JMC * _Nullable jmc, BOOL success);

/**
 *  Default PGDCore setup for JMC Operations, Everything is included Concept and with Block to handle Async Operations.
 *
 *  @author                Andy Newby
 *  @param customFields    Optional Application-Specific key-value pairs, All default keys are already included.  Assert Logs will help prevent incorrect setup. (View Logs)
 *  @param completionBlock Optional Completion Block to hande any after setup executions.
 */
